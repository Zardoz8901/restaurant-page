export function menuRender() {
    ...
    menu.setAttribute('id', 'menu-section');
    for (let i = 0; i < 8; i += 1) {
...
export function menuPopulate() {
    ...
    menuItemsArray.forEach((e, i) => {
        ...
        div.innerText = capitalizeWith(menuItems[i]);
        e.appendChild(div);
    });
    ...
    const menuDescriptionsArray = Array.from(document.querySelectorAll('.menu-item-description'));
